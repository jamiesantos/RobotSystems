...
class Interpreter:
    ...
    def process(self, readings: Iterable[float]) -> float:
        """Return offset in [-1, 1]. Positive -> line is left of robot.
    ...
    try:
        ...
        while True:
            ...
            if smoothing_alpha and smoothing_alpha > 0.0:
                ...
                use_offset = ema_offset
            else:
                use_offset = offset

            last_angle = controller.command(use_offset)
            if verbose:
                print(f"[line_control] readings={readings}, raw_offset={offset:.3f}, ema_offset={ema_offset if ema_offset is not None else 'N/A'}, angle={last_angle}", flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--speed", type=int, default=30, help="forward speed when running on a car")
